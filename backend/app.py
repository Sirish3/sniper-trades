...
# (https://stockpilot.cc in production, http://localhost:5173 in dev) via
# fetch(), not from a same-origin proxy — so the browser enforces CORS.
ALLOWED_ORIGINS = {"https://stockpilot.cc", "http://localhost:5173"}
...
@app.route("/api/qqq-signal/send", methods=["OPTIONS"])
@app.route("/api/positions/manual", methods=["OPTIONS"])
@app.route("/api/alert/screener-buy", methods=["OPTIONS"])
def cors_preflight():
    return "", 204
